...
func TestTLSCACertificatesFromSupport(t *testing.T) {
	...
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			certs, err := TLSCACertificatesFromSupport(cs)
			if testCase.expectedError == "" {
...
func TestConsenterEndpoints(t *testing.T) {
	_, err := consenterEndpoints([]*etcdraft.Consenter{
		nil,
	})
	assert.EqualError(t, err, "nil consenter found in metadata")

	consenters, err := consenterEndpoints([]*etcdraft.Consenter{
		{Host: "foo", Port: 7051, ServerTlsCert: []byte{1, 2, 3}},
		{Host: "bar", Port: 8051, ClientTlsCert: []byte{1, 2, 3}},
	})
	assert.NoError(t, err)
	assert.Equal(t, []string{"foo:7051", "bar:8051"}, consenters)
}
...
func TestNewBlockPuller(t *testing.T) {
	...
	bp, err := newBlockPuller(cs, consenters, dialer, localconfig.Cluster{})
	assert.NoError(t, err)
	assert.NotNil(t, bp)
	...
	for _, testCase := range []struct {
	...
	}{
		{
			...
			dialer:        dialer,
			expectedError: "nil consenter found in metadata",
		},
		{
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			bp, err := newBlockPuller(testCase.cs, testCase.consenters, testCase.dialer, localconfig.Cluster{})
			assert.Nil(t, bp)
			assert.EqualError(t, err, testCase.expectedError)
		})
	}
