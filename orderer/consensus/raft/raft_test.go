	...
	Describe("submit a message", func() {
		...
		Context("when there is only one peer", func() {
			...
			BeforeEach(func() {
				...
				fakeRaftSupport.NodeIDReturns(uint64(1))
			...
			It("fails if no leader exists", func() {
				cluster := raft.NewRaftCluster(fakeRaftSupport)
				cluster.Start()
				defer cluster.Stop()

				done := make(chan struct{})
				go func() {
					Consistently(cluster.SendC()).ShouldNot(Receive())
					close(done)
				}()
				Expect(cluster.Process(&orderer.SubmitRequest{})).Should(HaveOccurred())
				<-done
			})
			...
			It("succeeds if this is leader", func() {
				...
				go func() {
					defer GinkgoRecover()
					Eventually(cluster.SendC()).Should(Receive())
					close(done)
				}()
				tickNTimes(13, signalC)
				Eventually(func() error {
					return cluster.Process(&orderer.SubmitRequest{})
				}).Should(Succeed())
				<-done
			})
		})
