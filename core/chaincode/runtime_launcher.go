...
type ConnectionHandler interface {
	Run(ccid string, ccinfo *ccintf.ChaincodeServerInfo, sHandler extcc.StreamHandler) error
}
...
func (r *RuntimeLauncher) Launch(ccid string, streamHandler extcc.StreamHandler) error {
	...
	if !alreadyStarted {
		...
		go func() {
			...
			if ccservinfo != nil {
				if err = r.ConnectionHandler.Run(ccid, ccservinfo, streamHandler); err != nil {
					startFailCh <- errors.WithMessage(err, fmt.Sprintf("connection to %s failed", ccid))
					return
				}
