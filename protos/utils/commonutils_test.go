...
func TestIsConfigBlock(t *testing.T) {
	makeSingleEnvelopeBlock := func(env *cb.Envelope) *cb.Block {
		return &cb.Block{
			Data: &cb.BlockData{
	...
	makeConfigEnvelope := func(envType int32) *cb.Envelope {
		return &cb.Envelope{
	...
	env := makeConfigEnvelope(envType)
	block := makeSingleEnvelopeBlock(env)
