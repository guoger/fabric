...
var _ = Describe("EndToEnd Crash Fault Tolerance", func() {
	...
	When("Leader cannot reach quorum", func() {
		It("Steps down", func() {
			...
			By(fmt.Sprintf("Killing two followers (%d and %d)", followerIndeces[0]+1, followerIndeces[1]+1))
			ordererProcesses[followerIndeces[0]].Signal(syscall.SIGTERM)
			ordererProcesses[followerIndeces[1]].Signal(syscall.SIGTERM)

			By("Waiting for followers to die")
			Eventually(ordererProcesses[followerIndeces[0]].Wait(), network.EventuallyTimeout).Should(Receive())
			Eventually(ordererProcesses[followerIndeces[1]].Wait(), network.EventuallyTimeout).Should(Receive())

			By("Waitinf for leader to step down")
			...
			By("Performing operation with leader")
